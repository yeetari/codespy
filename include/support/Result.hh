...
#define JAMF_EXPECT(expr, ...)                                                                                         \
    jamf::maybe_unwrap(({                                                                                              \
        auto _result_expect = (expr);                                                                                  \
        assert(!_result_expect.is_error() __VA_OPT__(, ) __VA_ARGS__);                                                 \
        _result_expect.disown_value();                                                                                 \
    }))

#define JAMF_TRY(expr)                                                                                                 \
    jamf::maybe_unwrap(({                                                                                              \
...
public:
    ...
    Result(Variant<Es...> &&error) : m_value(move(error)) {}

    template <typename U>
    Result(U &ref)
        requires(is_ref<T> && !is_void)
        : m_value(jamf::ref(ref)) {}
    template <typename U>
    Result(const U &ref)
        requires(is_ref<T> && !is_void)
        : m_value(jamf::cref(ref)) {}
    ...
    auto &value() {
        assert(!is_error());
        return jamf::maybe_unwrap(m_value.template get<storage_t>());
    }
